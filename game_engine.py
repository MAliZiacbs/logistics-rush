# File: logistics-rush/game_engine.py
...
def process_location_checkin(location):
    """Process a player checking in at a location with error handling"""
    ...
    try:
        if not st.session_state.current_route:
            st.session_state.current_route = [location]
            return None
        ...
        st.error(f"An error occurred during location check-in: {e}")
        return None

def pickup_package(package):
    """Pick up a package at the current location with error handling"""
    if not st.session_state.game_active:
        st.warning("Please start a new game first!")
        return
    if not package:
        st.error("Invalid package selected for pickup.")
        return
    try:
        ...
        st.error(f"An error occurred while picking up the package: {e}")
...
def end_game():
    """End the game and calculate results with error handling"""
    if not st.session_state.game_active:
        return None
    ...
    try:
        ...
        _, optimal_distance = calculate_route_distance(st.session_state.optimal_route, graph)
        ...
        st.error(f"An error occurred during game end: {e}")
        return None
